Leafsnap Dataset - subset of 30 species Readme
-----------------------------------------------

For the purpose of a demo a subset of top 30 species (in terms of total number of "lab" and "field" images) has been selected from the original Leafsnap dataset of 185 species.
Column 'segmented path' has been removed as segmented images are not part of the dataset subset selection.
The lab images have been cropped semi-manually using IrfanView to remove the rulers and color calibration image parts. 
This results in a dataset of 7395 images. The original lab images are also provided, but the description file contains the path only to the cleaned 
lab images.

Below is the text of the original README.

Leafsnap (http://leafsnap.com- the web site is not responsive anymore in end August 2020) is an electronic field guide for identifying tree
...
The full dataset covers all 185 tree species from the Northeastern United States. 
...
- The source of the image. This is originally either 'lab' or 'field', for this subset- only 'lab'
